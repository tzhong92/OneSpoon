...
@implementation OSPFavoriteViewController {
  NSArray<OSPProfile *> *_favoriateProfiles;
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
  return [_favoriateProfiles count];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  OSPProfile *profile = _favoriateProfiles[indexPath.row];
  return [[OSPFavoriteTableViewCell alloc] initWithProfile:profile];
}
...
- (void)setUpDefaultData {
  ...
  profile3.isLikeEachOther = YES;
  [profiles addObject:profile3];
  
  _favoriateProfiles = [NSArray arrayWithArray:profiles];
}
